      ...
      try {
        ...
        if (!appointmentId) {
          ...
          if (res) {
            bot.sendText(
              msg.from,
              `An appointment has been already booked with your appointment Id. Check for Details Below. ${process.env.WEB_URL}/reschedule-appointment/?appointmentId=${appointmentId}`,
              { preview_url: true }
            );
          }
        }
  ...
  if (response) {
    ...
    if (data) {
      let dataObjjjj = {
        ...
        tmp_data: {
          ...userExist.tmp_data,
          Booking_slip: `${process.env.WEB_URL}/reschedule-appointment/?appointmentId=${data.appointment_ids[0]}`,
        },
      };
      ...
      if (resss) {
        ...
        bot.sendText(
          msg.from,
          `Thank you for booking the appointment. We have emailed you the appointment booking slip. For more details, click here: ${process.env.WEB_URL}/reschedule-appointment/?appointmentId=${data.appointment_ids[0]}`,
          { preview_url: true }
        );
      }
    }
